...
const SettingItem = React.memo(
	({ icon, title, isDestructive = false }: any) => (
...
const UserProfile = () => {
	...
	const progressWidth: DimensionValue = isPrepaid
		? `${(parseFloat(String(user?.member?.balance) || "0") / parseFloat(String(user?.member?.membershipPlan.price) || "1")) * 100}%`
		: "70%";
		...
		<>
			...
			<View className="items-center mt-6 mb-8">
				<View className="relative">
					<View className="w-24 h-24 bg-neon/20 rounded-full justify-center items-center border-2 border-neon">
			...
			<View className="bg-[#111A18] p-5 rounded-2xl border border-neon/30 mb-8 relative overflow-hidden">
				...
				<Text className="text-white font-header-bold text-xl mb-4 capitalize">
					{user?.member?.membershipPlan.type} Pass
				</Text>
				...
				<View className="mb-4">
					<View className="flex-row justify-between mb-2">
						...
						<Text className="text-white font-body-semibold text-xs">
							{isPrepaid
								? `₱${user?.member?.balance} / ₱${user?.member?.membershipPlan.price}`
								: "3 days left"}
						</Text>
					</View>
